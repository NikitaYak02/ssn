...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SEEM runtime bridge for interactive_benchmark.")
...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    width = max(4, int(round(min(h, w) * 0.012)))
    ...
    for item in prompts:
        prompt_type = str(item.get("prompt_type") or "scribble").lower()
        points = item.get("points") or []
        points_xy = [point_to_pixel(pt, h, w) for pt in points]
        if prompt_type not in {"scribble", "line", "point"}:
            continue
        if prompt_type == "point":
            cur = rasterize_points_gpu(h=h, w=w, points_xy=points_xy[:1], radius_px=width, device=device)
        else:
            cur = rasterize_polyline_gpu(h=h, w=w, points_xy=points_xy, width_px=width, device=device)
        agg |= cur
