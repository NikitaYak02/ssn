...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Semantic-SAM runtime bridge for interactive_benchmark.")
...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    radius = max(6, int(round(min(h, w) * 0.015)))
    ...
    for item in prompts:
        ...
        points_xy = [point_to_pixel(pt, h, w) for pt in points[:1]]
        out |= rasterize_points_gpu(h=h, w=w, points_xy=points_xy, radius_px=radius, device=device)
    arr = out.detach().cpu().numpy().astype(bool)
    if device.type == "cuda":
